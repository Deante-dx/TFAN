...
def regress_pred(model, pbar, num_samples, joint_used_xyz, m_p3d_h36, act):
    ...
    for (motion_input, motion_target) in pbar:
        ...
        for idx in range(num_step):
            with torch.no_grad():
                ...
                bone_pred = bone_pred + bone_input[:, -1:, :].repeat(1, step, 1).cuda()
...
def test(config, model, dataloader, act) :
    ...
    pbar = dataloader
    m_p3d_h36 = regress_pred(model, pbar, num_samples, joint_used_xyz, m_p3d_h36, act=act)
...
if __name__ == "__main__":
    model_pth = osp.join(config.root_dir, 'checkpoints/h36m/model_bone.pth')
    ...
    print(test(config, model, dataloader, act=action[act_idx]))
