...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter) :
    ...
    bone_target = h36m_motion_target.reshape(-1, 10, 25, 3)
    start_node = bone_target[:, :, strat_idx]
    end_node = bone_target[:, :, end_idx]
    bone_target = end_node - start_node
    ...
    bone_pred = torch.matmul(idct_m[:, :config.motion.h36m_input_length, :], bone_pred)[:, :10]
    offset_ = bone_input[:, -1:].cuda()
    bone_pred = bone_pred[:, :config.motion.h36m_target_length] + offset_

    loss_bone = get_bone_loss(bone_target, bone_pred)  #骨头向量损失
    bone_target_v = gen_velocity(Rearrange('b n j a -> b n (j a)')(bone_target))
...
while (nb_iter + 1) < config.cos_lr_total_iters:
    for (h36m_motion_input, h36m_motion_target) in dataloader:
        ...
        if (nb_iter + 1) % config.save_every == 0:
            ...
            model.eval()
            acc_tmp = test(eval_config, model, eval_dataloader, act='all')
            print(acc_tmp)
