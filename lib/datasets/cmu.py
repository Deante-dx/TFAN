...
import numpy as np
import lib.datasets.data_utils as data_utils
import torch


class CMU_Motion3D(Dataset):

    def __init__(self, path_to_data, actions, input_n=10, output_n=10, split=0, data_mean=0, data_std=0):
        ...
        all_seqs = all_seqs[:, :, dim_used]  #使用25個節點
        ...
        self.input = all_seqs[:, :input_n]/1000
        self.output = all_seqs[:, input_n:]/1000

    def __len__(self):
        return np.shape(self.input)[0]

    def __getitem__(self, item):
        # i = self.input[item].shape[0]
        # motion = np.concatenate([self.input[item], self.output[item]], axis=0)
        # if self.split == 0:
        #     if torch.rand(1)[0] > .5:
        #         idx = [i for i in range(motion.shape[0]-1, -1, -1)]
        #         idx = torch.LongTensor(idx)
        #         motion = motion[idx]
        # return motion[:i], motion[i:]
