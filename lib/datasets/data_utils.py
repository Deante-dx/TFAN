...
import os
import lib.datasets.forward_kinematics as forward_kinematics
...
def define_actions_humanact(action):
    actions = [
        "warm_up",
        "walk",
        "run",
...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    if is_test:
        ...
        # joint_to_ignore = np.array([1, 7, 13, 16, 20, 29, 24, 27, 33, 36])
    else:
        joint_to_ignore = np.array([0, 1, 2, 7, 8, 13, 16, 20, 29, 24, 27, 33, 36])
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    action_sequence = readCSVasFloat(filename)
                    n, d = action_sequence.shape
                    even_list = range(0, n, sample_rate)     #采样动作帧
                    the_sequence = np.array(action_sequence[even_list, :])
                    ...
                    for i in np.arange(seq_len - 1):
                        fs_sel = np.vstack((fs_sel, fs + i + 1))
                    fs_sel = fs_sel.transpose()
                    seq_sel = the_sequence[fs_sel, :]      #得到了长度为(input_n + output_n)帧的样本，每个样本都是从原样本中抽取出来的，从原样本中抽取nframes-（input_n+output_n）+1个小样本
...
def load_data_3d(path_to_dataset, subjects, actions, sample_rate, seq_len):
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    action_sequence = readCSVasFloat(filename)
                    n, d = action_sequence.shape
                    even_list = range(0, n, sample_rate)    #采样  （隔一帧采样一次）
                    num_frames = len(even_list)   #采样后的帧数
                    the_sequence = np.array(action_sequence[even_list, :])    #采样后的motion
                    the_seq = Variable(torch.from_numpy(the_sequence)).float().cuda()   #放到gpu中
                    # remove global rotation and translation
                    the_seq[:, 0:6] = 0    #这里要搞清楚数据集中的具体情况才能知道   应该和全局的信息有关
                    p3d = expmap2xyz_torch(the_seq)     #轴角转换成xyz
                    the_sequence = p3d.view(num_frames, -1).cpu().data.numpy()

                    fs = np.arange(0, num_frames - seq_len + 1)    #表示 num_frames长的motion序列 可以划分成fs个seq_len长的 motion
                    fs_sel = fs
                    for i in np.arange(seq_len - 1):
                        fs_sel = np.vstack((fs_sel, fs + i + 1))
                    fs_sel = fs_sel.transpose()      #转置
                    seq_sel = the_sequence[fs_sel, :]     #将长的序列 划分成多个短的保存到seq_sel
                    if len(sampled_seq) == 0:
                        sampled_seq = seq_sel
                        complete_seq = the_sequence
                    else:
    ...
    joint_to_ignore = np.array([0, 1, 6, 11, 16, 20, 23, 24, 28, 31])    #不要的关节点
    dimensions_to_ignore = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    dimensions_to_use = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)   #np.setdiff1d 找到两个集合的差异
