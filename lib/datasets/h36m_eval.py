...
class H36MEval(data.Dataset):
    def __init__(self, config, split_name, paired=True, act='all'):
        ...
        self.select_act = self._select_act(act)
        self._file_length = len(self.data_idx)



    def _select_act(self, act):
        if act == 'all':
            return
        else:
            idx = self._actions.index(act)
            # idx_start = idx * 2
            # idx_end = idx_start + 2
            # self.h36m_seqs = self.h36m_seqs[idx_start: idx_end]
            idx_start = idx * 256
            idx_end = idx_start + 256
            self.data_idx = self.data_idx[idx_start: idx_end]
